  ...
  get(filters)
    .forEach((filter: Filter) => {
      if(filter.filterType === 'snp'){
        convertSnpFilterToClause(filter)
          .forEach((genomicFilter) => {
            query.genomicFilters.push(genomicFilter);
          });
      } else if(filter.filterType === 'genomic'){
        convertGenomicFilterToClause(filter)
          .forEach((genomicFilter) => {
            query.genomicFilters.push(genomicFilter);
          });
      } else {
        baseClause.phenotypicClauses.push(
...
function addAuthorizationFiltersV3(
  query: QueryV3, 
  addConsents: boolean,
){
...
const convertPhenotypicFilterToClause = (
  filter: Filter
