    ...
    fetchingKind[kind] = (
      ...
      .then((results: ConfigObject[] | null) => {
        if (results === null) {
          ...
          // Stamp with the time the fetch actually completed, not when it was
          // requested - startup cooldown and backoff retries can both push
          // completion well past `now`, and anchoring to `now` would shorten
          // the cache's effective TTL by however long that took.
