<script lang="ts">
  ...
  class RemoteTreeNodeClass implements TreeNodeInterface {
    ...
    constructor(apiNode: SearchResult) {
      ...
      this.isLeaf = this.determineIfLeaf(apiNode);
    ...
    private determineIfLeaf(node: SearchResult): boolean {
  ...
  let treeNodes: RemoteTreeNodeClass[] = $state(initialNodes.map(node => new RemoteTreeNodeClass(node)));
</script>
