<script lang="ts">
  ...
  let { fadeEffect = false, numCategories = 2, previousCategories = [] as PreviousCategoriesForPlaceholder[] } = $props();
...
<div class="flex flex-col gap-4">
  {#if smartMode}
    {#each previousCategories as category, i}
      <FacetPlaceholder numFacets={category?.numFacets || 5} fadeEffect={fadeLastCategory && (i === previousCategories.length - 1)}/>
    {/each}
  {:else}
    {#each Array(numberOfCategories) as _, i}
      <FacetPlaceholder numFacets={numFacets - i > 1 ? numFacets - i : 2} fadeEffect={fadeLastCategory && (i === numberOfCategories - 1)} />
    {/each}
  {/if}
</div>