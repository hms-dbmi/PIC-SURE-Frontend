import { s as subscribe, d as compute_slots } from './utils-EiTRXYbg.js';
import { c as create_ssr_component, e as escape, v as validate_component, f as createEventDispatcher, a as add_attribute, b as each, s as setContext, g as getContext } from './ssr-C099ZcAV.js';
...
import { c as consequences, s as selectedFrequency, a as selectedGenes, b as clearGeneFilters, d as selectedConsequence, v as variantData } from './GeneFilter-DO0g_qs1.js';
...
const FilterType = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${"flex flex-row justify-center justify-items-center gap-16 " + escape($$props.class ?? "", true)}">${features.enableGENEQuery ? `${validate_component(CardButton, "CardButton").$$render(
    $$result,
    {
    ...
    {},
    {}
  )}` : ``} ${features.enableSNPQuery ? `${validate_component(CardButton, "CardButton").$$render(
    $$result,
    {
    ...
    {},
    {}
  )}` : ``} ${!features.enableGENEQuery && !features.enableSNPQuery ? `Genomic filtering has not been enabled in this environment. :(` : ``}</div>`;
});
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let selectedOption = ["snp", "genomic"].includes(edit) ? edit : Option.None;
  function clearFilters() {
    clearGeneFilters();
    clearSnpFilters();
  }
  canComplete = selectedOption === Option.Genomic && ($selectedGenes.length > 0 || $selectedFrequency.length > 0 || $consequences.length > 0) || selectedOption === Option.SNP && $selectedSNPs.length > 0;
  $$unsubscribe_selectedSNPs();
  $$unsubscribe_consequences();
  $$unsubscribe_selectedFrequency();
  $$unsubscribe_selectedGenes();
  ...
  return `${$$result.head += `<!-- HEAD_svelte-xe0801_START -->${$$result.title = `<title>${escape(branding.applicationName)} | Gemonic Filter</title>`, ""}<!-- HEAD_svelte-xe0801_END -->`, ""} ${validate_component(Content, "Content").$$render(
    ...
    {
      default: () => {
        return `${selectedOption === Option.None ? `${validate_component(FilterType, "FilterType").$$render($$result, { class: "my-4", active: selectedOption }, {}, {})}` : `${validate_component(FilterType, "FilterType").$$render($$result, { class: "my-4", active: selectedOption }, {}, {})} ${selectedOption === Option.Genomic ? `${validate_component(GeneSearch, "GeneSearch").$$render($$result, { class: "mb-0 mt-6" }, {}, {})}` : `${validate_component(SNPSearch, "SnpSearch").$$render($$result, { class: "mt-6" }, {}, {})}`} ${edit ? `<div class="flex justify-end my-4"><button data-testid="save-filter-btn" type="button" class="btn btn-sm variant-filled-primary text-lg disabled:opacity-75" ${!canComplete ? "disabled" : ""}>Save Filter <i class="fa-solid fa-plus ml-3"></i></button></div>` : `<div class="flex justify-end my-4" data-testid="filter-options-container"><button data-testid="add-filter-btn" type="button" class="btn btn-sm variant-filled-primary text-lg disabled:opacity-75" ${!canComplete ? "disabled" : ""}>Add Filter <i class="fa-solid fa-plus ml-3"></i></button></div>`}`}`;
      }
    }
  )}`;
});
...
//# sourceMappingURL=_page.svelte-BQoJOYq1.js.map
