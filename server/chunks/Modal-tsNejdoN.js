import { x as push, N as attr, M as attr_class, P as stringify, O as clsx, Y as bind_props, z as pop, Q as props_id, S as spread_attributes, K as escape_html } from './index-C5NonOVO.js';
import { r as raf$1, s as getDocument, v as getTabbables, x as getFocusables, y as isTabbable, z as getTabIndex, A as addDomEvent, B as getEventTarget, C as isDocument, D as isFocusable, E as getActiveElement, F as setStyleProperty, G as setStyle, H as isIos, a as createMachine, J as getComputedStyle, K as trackDismissableElement, h as createAnatomy, L as mergeProps$1 } from './User-ByrNDeqq.js';
import { c as createProps, u as useMachine, t as toStyleString, n as normalizeProps } from './Loading-Drx6gnkR.js';
...
function Modal_1($$payload, $$props) {
  ...
  $$payload.out += `<!--]--> `;
  {
    let content = function($$payload2) {
      ...
      if (withDefault) {
        $$payload2.out += "<!--[-->";
        $$payload2.out += `<article class="modal-body overflow-hidden">`;
        children?.($$payload2);
        $$payload2.out += `<!----></article> `;
        if (footerButtons) {
          $$payload2.out += "<!--[-->";
          $$payload2.out += `<footer class="modal-footer flex justify-end space-x-2 mt-6"><button type="button"${attr_class(`btn ${stringify(cancelClass)}`)}>${escape_html(cancelText)}</button> `;
          if (onconfirm) {
            $$payload2.out += "<!--[-->";
            $$payload2.out += `<button type="button"${attr_class(`btn ${stringify(confirmClass)}`)}>${escape_html(confirmText)}</button>`;
          } else {
            $$payload2.out += "<!--[!-->";
          }
          $$payload2.out += `<!--]--></footer>`;
...
//# sourceMappingURL=Modal-tsNejdoN.js.map
