import { M as escape_html, _ as current_component, x as push, R as props_id, T as spread_attributes, Q as stringify, N as attr_class, O as attr, z as pop, $ as run, F as noop } from './index-C9dy-hDW.js';
import { a as createScope, b as compact, e as ensure, M as MachineStatus, d as createMachine, f as isFunction, h as identity, w as warn, j as toArray, k as isString, I as INIT_STATE, l as memo, n as isNumber, o as getValuePercent, p as createAnatomy } from './User-CeJunCPd.js';
...
function useMachine(machine2, userProps) {
  ...
  const props = machine2.props?.({ props: compact(access(userProps)), scope }) ?? access(userProps);
  ...
  const getState = () => ({
    ...
    hasTag(tag) {
      ...
      return !!machine2.states[currentState]?.tags?.includes(tag);
    },
    matches(...values) {
      const currentState = state.get();
  ...
  const send = (event) => {
    ...
    const transitions = machine2.states[currentState].on?.[event.type] ?? machine2.on?.[event.type];
...
//# sourceMappingURL=Loading-Bei-CWQ1.js.map
