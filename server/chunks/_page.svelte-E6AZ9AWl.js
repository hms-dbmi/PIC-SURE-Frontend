import { s as subscribe, e as is_promise, n as noop, c as null_to_empty } from './utils-EiTRXYbg.js';
import { c as create_ssr_component, v as validate_component, e as escape, b as each, a as add_attribute } from './ssr-C099ZcAV.js';
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<section id="logins" class="flex flex-col items-center h-screen w-full text-center place-content-center text-lg"><div id="title-box" class="flex flex-col items-center text-center mb-8"><h1 data-testid="login-title" class="mb-6 w-full flex gap-2 items-center justify-center">${validate_component(Logo, "Logo").$$render($$result, { class: "flex-none", height: 4 }, {}, {})}</h1> <p data-testid="login-description" class="max-w-16 text-2xl">${escape(description)}</p></div> ${function(__value) {
    ...
    return function(providers) {
      ...
      })}`}</div> ${features.login.open ? `<a${add_attribute("href", branding?.login?.openPicsureLink || "/", 0)} class="btn variant-outline-primary hover:variant-filled-primary text-primary-500 m-1 mt-2 w-full mb-1">${escape(openPicsureLinkText)}</a>` : ``} ${function(__value2) {
...
//# sourceMappingURL=_page.svelte-E6AZ9AWl.js.map
