import { x as push, a2 as spread_props, z as pop, a3 as run, a0 as derived } from './index-C5NonOVO.js';
import { a as getDefaultRows, V as ViewBuilder, E as EventDispatcher } from './Dictionary-10axK71X.js';
...
const data = {
  ...
  query: (allRows, { path, key, value, check: check2 }) => {
    return allRows.map((row) => {
      ...
      for (const prop of path) {
        if (obj[prop]) {
          if (obj[root]) {
            obj[root] = data.query(obj[root], { path, key, value, check: check2 });
          }
          if (i < props.length) {
            obj[prop] = data.query(obj[prop], { path, key, value, check: check2 });
          } else {
            obj[prop] = obj[prop].filter((item) => check2(item[key], value));
          }
        }
        i++;
      }
...
class QueryHandler {
  ...
  set(path, key, value, check2, id) {
    this.table.setPage(1);
    this.table.queries = this.table.queries.filter((query) => query.id !== id);
    if (isNotNull(value)) {
      this.table.queries.push({ path, key, value, check: check2, id });
    }
  }
  unset(id) {
    this.table.setPage(1);
...
class QueryBuilder {
  ...
  set(value) {
    ...
    this.queryHandler.set(this.path, this.key, this.value, this.check ?? check.isLike, this.id);
  }
  clear() {
    this.value = "";
...
class AdvancedFilterBuilder {
  ...
  set(value, check$1) {
    if (this.collection.find((criterion) => criterion.value === value)) {
      ...
      this.collection = [
        { value, check: check$1 ?? this.check },
        ...this.collection
      ];
    }
...
class CalcultationBuilder {
  ...
  median(param) {
    this.precision = param?.precision ?? 2;
    const values = [
      ...this.table.allRows.map((row) => this.callback(row))
    ].sort((a, b) => a - b).filter(Boolean);
...
function StaticTable($$payload, $$props) {
  ...
  const handler = new TableHandler(data2, {
...
//# sourceMappingURL=StaticTable-h7VnOZfV.js.map
