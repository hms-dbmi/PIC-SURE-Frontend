import { x as push, a8 as head, z as pop, M as escape_html, Y as await_block, S as ensure_array_like, U as store_get, O as attr, Q as stringify, X as unsubscribe_stores, N as attr_class } from './index-DMPVr6nO.js';
import { k as readable, o as derived } from './utils-B7NzVBxP.js';
...
import { t as toaster } from './User-01eW3TFo.js';
import { U as UsersStore, g as getUser, u as updateUser } from './Users-DX4768Wk.js';
import { C as ConnectionsStore, g as getConnection } from './Connections-Bb_icnG_.js';
import { R as RoleStore, g as getRole } from './Roles-Ddev91Of.js';
...
function Actions($$payload, $$props) {
  ...
  if (active) {
    $$payload.out.push("<!--[-->");
    $$payload.out.push(`<button${attr("data-testid", `user-${stringify(data.cell)}-edit-btn`)} type="button" title="Edit" aria-label="Edit" class="btn-icon-color"><i class="fa-solid fa-pen-to-square fa-xl"></i></button>`);
  } else {
    $$payload.out.push("<!--[!-->");
  }
  $$payload.out.push(`<!--]--> `);
  {
    let trigger = function($$payload2) {
      $$payload2.out.push(`<button${attr("data-testid", `user-${stringify(active ? "d" : "r")}eactivate-btn-${stringify(data.cell)}`)} type="button"${attr("title", `${stringify(active ? "D" : "R")}eactivate user`)}${attr("aria-label", `${stringify(active ? "D" : "R")}eactivate user`)} class="btn-icon-color"><i${attr_class(`fa-solid fa-trash${stringify(active ? "" : "-arrow-up")} fa-xl`)}></i></button>`);
    };
    Modal_1($$payload, {
      ...
      trigger,
      children: ($$payload2) => {
        $$payload2.out.push(`<!---->Are you sure you want to ${escape_html(active ? "d" : "r")}eactiveate user '${escape_html(data.row.email)}'?`);
      },
...
function Status($$payload, $$props) {
  ...
  $$payload.out.push(`<span${attr_class(`badge last:p-1 rounded-sm preset-filled-${data.cell === "Active" ? "success" : "error"}-500`)}>${escape_html(data.cell)}</span>`);
  pop();
}
    ...
    children: ($$payload2) => {
      await_block(
        ...
        () => {
          ...
          $$payload2.out.push(`<div class="flex gap-4 mb-6"><div class="flex-auto"><a data-testid="add-user-btn" class="btn preset-tonal-primary border border-primary-500 hover:preset-filled-primary-500" href="/admin/users/new">+ Add User</a></div></div> <!--[-->`);
          for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
            let connection = each_array[$$index];
            $$payload2.out.push(`<div${attr("id", `user-table-${connection.label.replaceAll(" ", "_")}`)} class="mb-10">`);
...
//# sourceMappingURL=_page.svelte-K1QTcTN3.js.map
