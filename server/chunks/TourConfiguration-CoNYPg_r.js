import { a as subscribe, h as null_to_empty, l as is_promise, n as noop, s as setContext, m as compute_slots, k as createEventDispatcher, f as getContext } from './lifecycle-DtuISP6h.js';
import { c as create_ssr_component, v as validate_component, e as escape, b as each, a as add_attribute, d as add_styles, h as add_classes } from './ssr-BRJpAXVH.js';
import { p as page } from './stores4-C3NPX6l0.js';
import { g as goto } from './client-BR749xJD.js';
import { b as branding, f as features } from './configuration-CJ60Yp9o.js';
import { s as searchTerm, e as error, a as selectedFacets, t as tour, b as tableHandler, l as loading, f as facetsPromise, c as searchPromise, h as hiddenFacets, S as SearchStore } from './Dictionary-BfHinM5z.js';
import { F as Filter, R as Row, C as Count, a as Rows, P as Pagination, b as activeTable, e as expandableComponents } from './Row-Cb2p0o0o.js';
import { c as clearExports, E as ExportStore } from './Export-lg6T-LMZ.js';
import { v4 } from 'uuid';
...
const RemoteTable = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="space-y-1">${title || searchable || $$slots.tableActions ? `<header class="${"flex items-center " + escape(
    ...
    true
  ) + " gap-4"}">${title ? `<div class="flex-auto"><h2 class="my-2">${escape(title)}</h2></div>` : ``} ${slots.tableActions ? slots.tableActions({}) : ``} ${searchable ? `<div class="flex-none">${validate_component(Search, "Search").$$render($$result, { handler }, {}, {})}</div>` : ``}</header>` : ``} <table id="${escape(tableName2, true) + "-table"}" data-testid="${escape(tableName2, true) + "-table"}" class="${"table table-auto table-hover align-middle " + escape(fullWidth ? "w-max" : "", true) + " svelte-em2rgy"}"><thead style="border-color: revert;"><tr>${each(columns, (column) => {
    return `${column.sort ? `${validate_component(Th, "Th").$$render(
      ...
      {},
      {}
    )}` : `<th class="${escape(null_to_empty(`bg-primary-300 ${column.class}`), true) + " svelte-em2rgy"}">${escape(column.label)}</th>`}`}`;
  })}</tr></thead> <tbody>${$isLoading ? `<tr><td${add_attribute("colspan", columns.length, 0)} class="text-center py-8"><div class="flex justify-center items-center">${validate_component(ProgressRadial, "ProgressRadial").$$render($$result, { width: "w-12" }, {}, {})}</div></td></tr>` : `${$rows.length > 0 ? `${each($rows, (row, i) => {
  ...
  })}` : `<tr><td${add_attribute("colspan", columns.length, 0)}>No entries found.</td></tr>`}`}</tbody></table> <footer class="flex justify-between mt-1">${validate_component(Count, "RowCount").$$render($$result, { handler }, {}, {})} <div class="flex justify-end gap-4">${validate_component(Rows, "RowsPerPage").$$render($$result, { handler, options }, {}, {})} ${validate_component(Pagination, "Pagination").$$render($$result, { handler }, {}, {})}</div></footer> </div>`;
});
const css$2 = {
  code: "input.indeterminate.svelte-1652d10{background-image:url('$lib/assets/dash.svg');background-size:100% 100%;background-repeat:no-repeat;background-position:center;background-color:rgb(var(--color-primary-500))}.arrow-button.svelte-1652d10{background-color:transparent}",
  map: '{"version":3,"file":"FacetItem.svelte","sources":["FacetItem.svelte"],"sourcesContent":["<script lang=\\"ts\\">import SearchStore from \\"$lib/stores/Search\\";\\nlet { updateFacets, selectedFacets } = SearchStore;\\nexport let facet;\\nexport let facetCategory;\\nexport let textFilterValue;\\nlet open = false;\\nfunction toggleOpen() {\\n  open = !open;\\n}\\nfunction getCurrentlySelectedChildren() {\\n  return facet.children?.filter((child) => $selectedFacets.some((f) => f.name === child.name)) ?? [];\\n}\\nfunction onClick() {\\n  if (facet?.children && facet?.children?.length > 0) {\\n    if (checkedState(facet.name) === \\"indeterminate\\") {\\n      updateFacets(getCurrentlySelectedChildren());\\n    } else {\\n      updateFacets(facet.children);\\n    }\\n  } else {\\n    updateFacets([facet]);\\n  }\\n}\\n$: checkedState = (facetToCheck) => {\\n  const atLeastOneChildSelected = facet.children && facet.children.some((child) => $selectedFacets.some((f) => f.name === child.name));\\n  let isEveryChildSelected = false;\\n  if (atLeastOneChildSelected) {\\n    isEveryChildSelected = facet.children?.every((child) => $selectedFacets.some((f) => f.name === child.name)) ?? false;\\n  }\\n  const isIndeterminate = atLeastOneChildSelected && !isEveryChildSelected;\\n  const isSelected = $selectedFacets.some((facet2) => facet2.name === facetToCheck) || isEveryChildSelected;\\n  if (isSelected)\\n    return \\"true\\";\\n  if (isIndeterminate)\\n    return \\"indeterminate\\";\\n  return \\"false\\";\\n};\\n$: checked = checkedState(facet.name) === \\"true\\";\\n$: facetsToDisplay = facet.children ? [\\n  ...facet.children.filter((child) => {\\n    const matchesFilter = !textFilterValue || child.display.toLowerCase().includes(textFilterValue.toLowerCase()) || child.name.toLowerCase().includes(textFilterValue.toLowerCase()) || child.description?.toLowerCase().includes(textFilterValue.toLowerCase());\\n    return matchesFilter && $selectedFacets.some((f) => f.name === child.name);\\n  }).sort((a, b) => (b.count || 0) - (a.count || 0)),\\n  ...facet.children.filter((child) => {\\n    const matchesFilter = !textFilterValue || child.display.toLowerCase().includes(textFilterValue.toLowerCase()) || child.name.toLowerCase().includes(textFilterValue.toLowerCase()) || child.description?.toLowerCase().includes(textFilterValue.toLowerCase());\\n    return matchesFilter && !$selectedFacets.some((f) => f.name === child.name);\\n  }).sort((a, b) => (b.count || 0) - (a.count || 0))\\n] : [];\\n<\/script>\\n\\n<label data-testId={`facet-${facet.name}-label`} for={facet.name} class=\\"m-1\\">\\n  {#if facet?.children !== undefined && facet?.children?.length > 0}\\n    <button\\n      type=\\"button\\"\\n      class=\\"arrow-button\\"\\n      aria-label=\\"Toggle Facet {open ? \'open\' : \'closed\'}\\"\\n      data-testId={`facet-${facet.name}-arrow`}\\n      on:click={toggleOpen}\\n    >\\n      <i class=\\"fa-solid {open ? \'fa-angle-down\' : \'fa-angle-right\'}\\"></i>\\n    </button>\\n  {/if}\\n  <input\\n    type=\\"checkbox\\"\\n    class={`&[aria-disabled=“true”]:opacity-75 ${checkedState(facet.name)}`}\\n    id={facet.name}\\n    name={facet.name}\\n    value={facet}\\n    {checked}\\n    disabled={facet.count === 0 && !checked}\\n    aria-checked={checked}\\n    on:click={onClick}\\n  />\\n  <span class:opacity-75={facet.count === 0}\\n    >{`${facet.display} (${facet.count?.toLocaleString()})`}</span\\n  >\\n</label>\\n{#if open && facetsToDisplay !== undefined && facetsToDisplay?.length > 0}\\n  <div class=\\"flex flex-col ml-4\\" data-testId={`facet-${facet.name}-children`}>\\n    {#each facetsToDisplay as child}\\n      <svelte:self facet={child} {facetCategory} {textFilterValue} />\\n    {/each}\\n  </div>\\n{/if}\\n\\n<style lang=\\"postcss\\">\\n  input.indeterminate {\\n    background-image: url(\'$lib/assets/dash.svg\');\\n    background-size: 100% 100%;\\n    background-repeat: no-repeat;\\n    background-position: center;\\n    background-color: rgb(var(--color-primary-500));\\n  }\\n\\n  .arrow-button {\\n    background-color: transparent;\\n  }</style>\\n"],"names":[],"mappings":"AAsFE,KAAK,6BAAe,CAClB,gBAAgB,CAAE,2BAA2B,CAC7C,eAAe,CAAE,IAAI,CAAC,IAAI,CAC1B,iBAAiB,CAAE,SAAS,CAC5B,mBAAmB,CAAE,MAAM,CAC3B,gBAAgB,CAAE,IAAI,IAAI,mBAAmB,CAAC,CAChD,CAEA,4BAAc,CACZ,gBAAgB,CAAE,WACpB"}'
};
const FacetItem = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let checkedState;
  let checked;
  let $selectedFacets, $$unsubscribe_selectedFacets;
  let { updateFacets, selectedFacets: selectedFacets2 } = SearchStore;
  $$unsubscribe_selectedFacets = subscribe(selectedFacets2, (value) => $selectedFacets = value);
  let { facet } = $$props;
  let { facetCategory } = $$props;
  let { textFilterValue } = $$props;
  if ($$props.facet === void 0 && $$bindings.facet && facet !== void 0) $$bindings.facet(facet);
  if ($$props.facetCategory === void 0 && $$bindings.facetCategory && facetCategory !== void 0) $$bindings.facetCategory(facetCategory);
  if ($$props.textFilterValue === void 0 && $$bindings.textFilterValue && textFilterValue !== void 0) $$bindings.textFilterValue(textFilterValue);
  ...
  return `<label${add_attribute("data-testid", `facet-${facet.name}-label`, 0)}${add_attribute("for", facet.name, 0)} class="m-1">${facet?.children !== void 0 && facet?.children?.length > 0 ? `<button type="button" class="arrow-button svelte-1652d10" aria-label="${"Toggle Facet " + escape("closed", true)}"${add_attribute("data-testid", `facet-${facet.name}-arrow`, 0)}><i class="${"fa-solid " + escape("fa-angle-right", true)}"></i></button>` : ``} <input type="checkbox" class="${escape(null_to_empty(`&[aria-disabled=“true”]:opacity-75 ${checkedState(facet.name)}`), true) + " svelte-1652d10"}"${add_attribute("id", facet.name, 0)}${add_attribute("name", facet.name, 0)}${add_attribute("value", facet, 0)} ${checked ? "checked" : ""} ${facet.count === 0 && !checked ? "disabled" : ""}${add_attribute("aria-checked", checked, 0)}> <span${add_classes((facet.count === 0 ? "opacity-75" : "").trim())}>${escape(`${facet.display} (${facet.count?.toLocaleString()})`)}</span></label> ${``}`;
});
const css$1 = {
...
const FacetCategory = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  const anyFacetNot0 = filteredHiddenFacets?.some((facet) => facet.count > 0);
  let textFilterValue;
  let { shouldShowSearchBar = filteredHiddenFacets?.length > numFacetsToShow } = $$props;
  let moreThanNumFacetsToShow = filteredHiddenFacets?.length > numFacetsToShow;
  function isIndeterminate(facet) {
    const atLeastOneChildSelected = facet.children?.some((child) => $selectedFacets.some((f) => f.name === child.name)) ?? false;
    const isEveryChildSelected = facet.children?.length ? facet.children.every((child) => $selectedFacets.some((f) => f.name === child.name)) : false;
    ...
    {},
    {
      content: () => {
        return `<div class="flex flex-col svelte-1ukmbqo">${shouldShowSearchBar ? `<input class="text-sm svelte-1ukmbqo" type="search"${add_attribute("placeholder", "Filter " + facetCategory.display, 0)} name="facet-fitler"${add_attribute("id", facetCategory.name + "-filter", 0)}${add_attribute("value", textFilterValue, 0)}>` : ``} ${each(facetsToDisplay, (facet) => {
          return `${validate_component(FacetItem, "FacetItem").$$render($$result, { facet, facetCategory, textFilterValue }, {}, {})}`;
        })} ${filteredHiddenFacets?.length > numFacetsToShow && !textFilterValue ? `<button data-testid="show-more-facets" class="show-more w-fit mx-auto my-1 svelte-1ukmbqo">${escape(moreThanNumFacetsToShow ? "Show More" : "Show Less")} <i class="${"ml-1 fa-solid " + escape(
          moreThanNumFacetsToShow ? "fa-angle-down" : "fa-angle-up",
          true
        ) + " svelte-1ukmbqo"}"></i></button>` : ``}</div> `;
      },
      summary: () => {
...
const FacetSideBar = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  $$unsubscribe_selectedFacets();
  $$unsubscribe_facetsPromise();
  return `${function(__value) {
    if (is_promise(__value)) {
      __value.then(null, noop);
      return ` <div class="radial-container svelte-b21ccw">${validate_component(ProgressRadial, "ProgressRadial").$$render($$result, { width: "w-10" }, {}, {})}</div> `;
    }
    return function(newFacets) {
      return ` ${newFacets?.length > 0 ? `${validate_component(Accordion, "Accordion").$$render($$result, { rounded: "rounded-container-token" }, {}, {
        default: () => {
          return `${each(newFacets, (facetCategory) => {
...
const ExplorerTour = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let $$unsubscribe_searchPromise;
  $$unsubscribe_searchPromise = subscribe(searchPromise, (value) => value);
...
const Explorer = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `<section id="search-container" class="flex gap-9"><div id="facet-side-bar" class="flex-none flex-col items-center w-64">${validate_component(FacetSideBar, "FacetSideBar").$$render($$result, {}, {}, {})}</div> <div id="search-results-col" class="flex-auto"><div id="search-bar" class="flex gap-2 mb-6"><div class="flex-auto">${validate_component(Searchbox, "Searchbox").$$render(
      $$result,
      { search: update, searchTerm: searchInput },
      {
        searchTerm: ($$value) => {
          searchInput = $$value;
          $$settled = false;
      ...
      {}
    )}</div> <div class="flex-none">${!isOpenAccess && (features.enableGENEQuery || features.enableSNPQuery) ? `<a data-testid="genomic-filter-btn" class="btn variant-ghost-primary hover:variant-filled-primary" href="/explorer/genome-filter" data-svelte-h="svelte-1nwljm2">Genomic Filtering</a>` : ``} <button type="button" class="btn variant-ghost-error hover:variant-filled-error" aria-label="You are on the reset button" ${!searchInput && $selectedFacets.length === 0 ? "disabled" : ""}>Reset</button></div></div> ${$error ? `${validate_component(ErrorAlert, "ErrorAlert").$$render(
      $$result,
      {
      ...
      {},
      {}
    )}` : ``}`} ${features.explorer.enableTour && $tour ? `<div id="explorer-tour" class="text-center mt-4">${validate_component(ExplorerTour, "ExplorerTour").$$render($$result, { tourConfig }, {}, {})}</div>` : ``}</div></section>`;
  } while (!$$settled);
...
//# sourceMappingURL=TourConfiguration-CoNYPg_r.js.map
