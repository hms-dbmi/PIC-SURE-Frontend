...
const Table = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="space-y-1">${title || search || $$slots.tableActions ? `<header class="${"flex items-center " + escape(
    ...
    true
  ) + " gap-4"}">${title ? `<div class="flex-auto"><h2 class="my-2">${escape(title)}</h2></div>` : ``} ${slots.tableActions ? slots.tableActions({}) : ``} ${search ? `<div class="flex-none">${validate_component(Search, "Search").$$render($$result, { handler }, {}, {})}</div>` : ``}</header>` : ``} <div class="overflow-x-auto"><table id="${escape(tableName, true) + "-table"}" data-testid="${escape(tableName, true) + "-table"}" class="${"table table-auto table-hover align-middle " + escape(fullWidth ? "w-max" : "", true) + " svelte-5n6op8"}"><thead><tr class="${escape(null_to_empty(stickyHeader ? "sticky-header" : ""), true) + " svelte-5n6op8"}">${each(columns, (column) => {
    return `${column.sort ? `${validate_component(Sort, "ThSort").$$render(
      ...
      {},
      {}
    )}` : `<th class="${escape(null_to_empty(`bg-primary-300 ${column.class}`), true) + " svelte-5n6op8"}">${escape(column.label)}</th>`}`}`;
...
export { Table as T };
//# sourceMappingURL=Table-BH3DMZ1c.js.map
