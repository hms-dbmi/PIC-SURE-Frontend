import { c as null_to_empty, s as subscribe, d as compute_slots, g as get_store_value, n as noop } from './utils-EiTRXYbg.js';
...
import { F as Filter, R as Row, C as Count, a as Rows, P as Pagination } from './Row-DCE9feR7.js';
...
const Sort = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_sorted();
  return `<th class="${"cursor-pointer select-none align-bottom " + escape($$props.class || "", true) + " svelte-1q8lxw8"}">${slots.default ? slots.default({}) : ``} ${$sorted.identifier === orderBy ? `${$sorted.direction === "asc" ? `↓` : `${$sorted.direction === "desc" ? `↑` : ``}`}` : `↕`} </th>`;
});
...
const Table = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="space-y-1">${title || search || $$slots.tableActions ? `<header class="${"flex items-center " + escape(
    ...
    true
  ) + " gap-4"}">${title ? `<div class="flex-auto"><h2 class="my-2">${escape(title)}</h2></div>` : ``} ${slots.tableActions ? slots.tableActions({}) : ``} ${search ? `<div class="flex-none">${validate_component(Search, "Search").$$render($$result, { handler }, {}, {})}</div>` : ``}</header>` : ``} <div class="overflow-x-auto"><table id="${escape(tableName, true) + "-table"}" data-testid="${escape(tableName, true) + "-table"}" class="${"table table-auto table-hover align-middle " + escape(fullWidth ? "w-max" : "", true) + " svelte-1pv2375"}"><thead><tr class="${escape(null_to_empty(stickyHeader ? "sticky-header" : ""), true) + " svelte-1pv2375"}">${each(columns, (column) => {
    return `${column.sort ? `${validate_component(Sort, "ThSort").$$render(
      ...
      {},
      {}
    )}` : `<th class="${escape(null_to_empty(`bg-primary-300 ${column.class}`), true) + " svelte-1pv2375"}">${escape(column.label)}</th>`}`}`;
  })}</tr></thead> <tbody class="svelte-1pv2375">${$rows.length > 0 ? `${each($rows, (row, i) => {
  ...
  })}` : `<tr><td${add_attribute("colspan", columns.length, 0)} class="svelte-1pv2375">No entries found.</td></tr>`}</tbody></table></div> <footer class="flex justify-between">${validate_component(Count, "RowCount").$$render($$result, { handler }, {}, {})} ${showPagination ? `<div class="flex justify-end gap-4">${validate_component(Rows, "RowsPerPage").$$render($$result, { options, handler }, {}, {})} ${validate_component(Pagination, "Pagination").$$render($$result, { handler }, {}, {})}</div>` : ``}</footer> </div>`;
});

export { Table as T };
//# sourceMappingURL=Table-0D_aobLH.js.map
