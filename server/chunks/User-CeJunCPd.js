import { w as writable, l as get$1, o as derived } from './utils-D3IkxnGP.js';
import { error } from '@sveltejs/kit';
import { B as BDCPrivileges, f as features, P as PicsurePrivileges, r as routes } from './configuration-BAm0JRx1.js';
import { g as goto } from './client2-BVaV_p61.js';
import { p as page } from './index2-CFqWCRce.js';
import { E as DEV } from './index-C9dy-hDW.js';
...
function groupConnect(service, normalize) {
  const { context, prop, send: send2, refs, computed } = service;
  return {
    getCount() {
    ...
    getGroupProps(options = {}) {
      ...
      return normalize.element({
        ...
        onMouseMove() {
          send2({ type: "REGION.POINTER_ENTER", placement });
        },
        onMouseLeave() {
          send2({ type: "REGION.POINTER_LEAVE", placement });
        },
        onFocus(event) {
          send2({ type: "REGION.FOCUS", target: event.relatedTarget });
        },
        onBlur(event) {
          if (refs.get("isFocusWithin") && !contains(event.currentTarget, event.relatedTarget)) {
            queueMicrotask(() => send2({ type: "REGION.BLUR" }));
          }
        }
      });
    },
...
var groupMachine = createMachine({
  ...
  implementations: {
    ...
    effects: {
      ...
      trackHotKeyPress({ prop, send: send2 }) {
        const handleKeyDown = (event) => {
          const { hotkey } = prop("store").attrs;
          const isHotkeyPressed = hotkey.every((key) => event[key] || event.code === key);
          if (!isHotkeyPressed) return;
          send2({ type: "DOC.HOTKEY" });
        };
        return addDomEvent(document, "keydown", handleKeyDown, { capture: true });
      },
      trackDocumentVisibility({ prop, send: send2, scope }) {
        const { pauseOnPageIdle } = prop("store").attrs;
        if (!pauseOnPageIdle) return;
        const doc = scope.getDoc();
        return addDomEvent(doc, "visibilitychange", () => {
          const isHidden = doc.visibilityState === "hidden";
          send2({ type: isHidden ? "PAUSE_ALL" : "RESUME_ALL" });
        });
      }
    },
    actions: {
      ...
      collapsedIfEmpty({ send: send2, computed }) {
        if (!computed("overlap") || computed("count") > 1) return;
        send2({ type: "REGION.OVERLAP" });
      },
...
function connect(service, normalize) {
  const { state, send: send2, prop, scope, context, computed } = service;
  ...
  return {
    ...
    dismiss() {
      send2({ type: "DISMISS", src: "programmatic" });
    },
    getRootProps() {
      return normalize.element({
        ...parts.root.attrs,
        ...
        onKeyDown(event) {
          if (event.defaultPrevented) return;
          if (event.key == "Escape") {
            send2({ type: "DISMISS", src: "keyboard" });
            event.preventDefault();
          }
        }
      });
...
var machine = createMachine({
  ...
  watch({ track, prop, send: send2 }) {
    track([() => prop("message")], () => {
      const message = prop("message");
      if (message) send2({ type: message, src: "programmatic" });
    });
  ...
  implementations: {
    effects: {
      waitForRemoveDelay({ prop, send: send2 }) {
        return setRafTimeout(() => {
          send2({ type: "REMOVE", src: "timer" });
        }, prop("removeDelay"));
      },
      waitForDuration({ send: send2, context, computed }) {
        if (computed("shouldPersist")) return;
        return setRafTimeout(() => {
          send2({ type: "DISMISS", src: "timer" });
        }, context.get("remainingTime"));
      },
      waitForNextTick({ send: send2 }) {
        return setRafTimeout(() => {
          send2({ type: "SHOW", src: "timer" });
        }, 0);
      },
  ...
  const promise = (promise2, options, shared = {}) => {
    ...
    const prom = runIfFn(promise2).then(async (response) => {
      ...
      if (options.error !== void 0) {
        removable = false;
        const errorOptions = runIfFn(options.error, error22);
        create({ ...shared, ...errorOptions, id, type: "error" });
      }
    }).finally(() => {
      if (removable) {
...
function isToastShowing(id) {
  return toaster.getVisibleToasts().find((toast) => toast.id === id) !== void 0;
}
...
const defaultResources = {
  ...
  search: __vite_import_meta_env__?.VITE_RESOURCE_SEARCH || __vite_import_meta_env__?.VITE_RESOURCE_HPDS || "",
  hpdsOpen: __vite_import_meta_env__?.VITE_RESOURCE_OPEN_HPDS || "",
  hpdsAuth: __vite_import_meta_env__?.VITE_RESOURCE_HPDS || "",
...
async function getResources() {
  ...
  if (features.federated) {
    await get(Picsure.Resources).then((siteResources) => {
      resourceMap.queryable = siteResources.filter(({ hidden, resourceRSPath }) => !hidden && resourceRSPath.includes("passthru")).map(({ name, uuid: uuid2 }) => ({ name, uuid: uuid2 }));
...
export { loadResources as $, isIos as A, getTabbables as B, getFocusables as C, isTabbable as D, getTabIndex as E, addDomEvent as F, getActiveElement as G, getEventTarget as H, INIT_STATE as I, isDocument as J, isFocusable as K, isUserLoggedIn as L, MachineStatus as M, userRoutes as N, observeAttributes as O, observeChildren as P, createGuards as Q, dispatchInputCheckedEvent as R, setElementChecked as S, trackFormControl as T, trackPress as U, dataAttr as V, visuallyHiddenStyle as W, isSafari as X, getWindow as Y, isVirtualClick as Z, isMac as _, createScope as a, loading as a0, isToastShowing as a1, getQueryResources as a2, post as a3, Picsure as a4, resources as a5, get as a6, isTopAdmin as a7, Psama as a8, del as a9, isAdmin as aa, put as ab, setup as ac, isAnchorElement as ad, trackElementRect as ae, nextTick as af, prevById as ag, nextById as ah, last as ai, first as aj, clickIfLink as ak, isOpeningInNewTab as al, isComposingEvent as am, contains as an, getEventKey as ao, itemById as ap, queryAll as aq, getTokenExpiration as ar, getTokenExpirationAsDate as as, getUser as at, refreshLongTermToken as au, add as av, remove as aw, browser as ax, compact as b, connect as c, createMachine as d, ensure as e, isFunction as f, group as g, identity as h, isLoggedIn as i, toArray as j, isString as k, memo as l, machine as m, isNumber as n, getValuePercent as o, createAnatomy as p, getComputedStyle as q, raf as r, trackDismissableElement as s, toaster as t, user as u, mergeProps as v, warn as w, getDocument as x, setStyleProperty as y, setStyle as z };
//# sourceMappingURL=User-CeJunCPd.js.map
