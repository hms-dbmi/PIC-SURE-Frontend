import { x as push, U as copy_payload, V as assign_payload, z as pop, T as store_get, W as unsubscribe_stores, Y as bind_props, P as stringify, _ as store_set, K as escape_html, M as attr_class, N as attr, R as ensure_array_like, X as await_block, Q as props_id, S as spread_attributes } from './index-C5NonOVO.js';
import { p as page } from './index3-D0mgFMjB.js';
import './client2-CLhyDddE.js';
import { a as createMachine, N as observeAttributes, O as observeChildren, h as createAnatomy, t as toaster, P as userRoutes, u as user, i as isLoggedIn } from './User-ByrNDeqq.js';
import { T as Toaster, F as Footer } from './Footer-DE3obZpr.js';
import { r as removeInvalidFilters, f as filterWarning, a as removeGenomicFilters, b as removeUnallowedFilters, c as filters, h as hasGenomicFilter, d as clearFilters } from './Filter-BUwQwcV6.js';
import { c as createProps, o as onDestroy, L as Loading, u as useMachine, n as normalizeProps } from './Loading-Drx6gnkR.js';
import { L as Logo } from './Logo-BFd7HApn.js';
import { P as Popover } from './Popover-D0sAJhG1.js';
import { e as exports, c as clearExports } from './Export-CTQ_v_3l.js';
import { f as features } from './configuration-CSskKBur.js';
import { h as hasNonZeroResult, r as resultCounts } from './ResultStore-CVyEAw3c.js';
import './GeneFilter-B5irLK_t.js';
import './index2-CvuFLVuQ.js';
import './client-BWx-wafP.js';
import { a as Modal, M as Modal_1 } from './Modal-tsNejdoN.js';
import { A as AddFilter } from './AddFilter-BbVq5aRW.js';
import { C as CardButton } from './CardButton-mV7OQcpn.js';
import { S as StatPromise, c as countResult } from './StatBuilder-Dl1Zy-p_.js';
import { p as panelOpen } from './SidePanel-RBU0AY7R.js';
import { o as open, a as activeRow } from './Dashboard-CZXlEPlJ.js';
import { g as getDatasetDetails } from './Dictionary-10axK71X.js';
      ...
      if (store_get($$store_subs ??= {}, "$user", user) && store_get($$store_subs ??= {}, "$user", user).privileges && store_get($$store_subs ??= {}, "$user", user).email && store_get($$store_subs ??= {}, "$isLoggedIn", isLoggedIn)) {
        $$payload2.out += "<!--[-->";
        {
          ...
          Popover($$payload2, {
            trigger,
            children: ($$payload3) => {
              $$payload3.out += `<div class="flex flex-col items-center"><p class="pb-6">${escape_html(store_get($$store_subs ??= {}, "$user", user).email)}</p> <button id="user-logout-btn" class="btn preset-filled-primary-500 w-fit" title="Logout">Logout</button></div>`;
            },
...
function Counts($$payload, $$props) {
  ...
  for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
    ...
    $$payload.out += `<div class="flex flex-col items-center mt-2">`;
    await_block(
      $$payload,
      ...
      (counts) => {
        ...
        $$payload.out += `<span id="result-count">`;
        if (counts.filter(StatPromise.fullfiled).length === 0) {
          $$payload.out += "<!--[-->";
          $$payload.out += `<span class="text-4xl font-bold">N/A</span>`;
        } else {
          $$payload.out += "<!--[!-->";
          $$payload.out += `<div class="flex flex-row h-full"><span id="result-count-number" class="text-4xl">${escape_html(count)}</span> `;
          if (hasError) {
            $$payload.out += "<!--[-->";
            $$payload.out += `<span id="result-count-error" class="text-xs ml-1 h-full !flex flex-col"><i class="fa-solid fa-circle-exclamation text-warning-900-100 pt-[5px] hover:text-warning-400-600" title="One or more sites had an error for this count."></i> <span class="sr-only">One or more sites had an error for this count.</span></span>`;
          } else {
            $$payload.out += "<!--[!-->";
          }
          $$payload.out += `<!--]--></div>`;
    ...
    $$payload.out += `<!--]--> <h4 class="text-center">${escape_html(stat.label)}</h4></div>`;
  }
  $$payload.out += `<!--]--> `;
  {
    $$payload.out += "<!--[!-->";
...
function ResultsPanel($$payload, $$props) {
  ...
  let showExportButton = features.explorer.allowExport && !isOpenAccess && hasFilterOrExport && store_get($$store_subs ??= {}, "$hasNonZeroResult", hasNonZeroResult);
  ...
  function $$render_inner($$payload2) {
    ...
    $$payload2.out += `<!----> <section id="results-panel" class="flex flex-col items-center pt-8 pr-10 w-64">`;
    Counts($$payload2);
    $$payload2.out += `<!----> `;
    if (showExportButton) {
      $$payload2.out += "<!--[-->";
      $$payload2.out += `<div class="h-11 mt-4"><button id="export-data-button" type="button" class="btn preset-filled-primary-500">Prepare for Analysis</button></div>`;
    } else {
      $$payload2.out += "<!--[!-->";
    }
    ...
    if (showToolSuite) {
      $$payload2.out += "<!--[-->";
      $$payload2.out += `<div class="flex flex-col items-center mt-7"><hr class="svelte-6n5r83"/> <h5 class="text-center text-xl mt-7">Tool Suite</h5> <div class="flex flex-row flex-wrap justify-items-center gap-4 w-80 justify-center">`;
...
//# sourceMappingURL=_layout.svelte-C7odlP-6.js.map
