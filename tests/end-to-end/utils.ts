...
import { conceptsDetailPath, facetResultPath, facetsResponse, searchResults } from './mock-data';
...
/**
 * Serves concept detail for any concept in `searchResults`, out of the row itself: the detail
 * page loads the concept before it renders anything, so every spec that opens a result needs
 * detail for it. Specs that assert on detail fields register their own route after this one
 * and win.
 */
