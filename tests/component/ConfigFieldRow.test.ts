...
function baseProps(schema: ConfigFieldSchema, apiMap: ConfigMap = {}) {
  return { schema, apiMap, envMap: {} as ConfigMap, apiAvailable: true, kind: 'features' as const };
}
...
describe('ConfigFieldRow', () => {
  ...
  it('prefers the API row value over env/default when origin is api', () => {
    ...
    render(ConfigFieldRow, baseProps(stringSchema, { [apiRow.name]: apiRow }));
